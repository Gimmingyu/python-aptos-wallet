...
class PublicKeyUtils:
    ...
    def derive_bip32child_key(self, parent_key, parent_chain_code, i):
        assert len(parent_key) == 32
        assert len(parent_chain_code) == 32
        k = parent_chain_code
        if (i and self.privdev) != 0:
    ...
    def mnemonic_to_private_key(self, mnemonic, passphrase=""):
        ...
        master_private_key, master_chain_code = self.bip39seed_to_bip32_master_node(bip39seed)
        private_key, chain_code = master_private_key, master_chain_code
        for i in derivation_path:
            private_key, chain_code = self.derive_bip32child_key(private_key, chain_code, i)
        return private_key

    def bip39seed_to_bip32_master_node(self, seed):
        k = seed
